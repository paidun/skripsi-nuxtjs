    ...
    <div class="container">
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center px-4">
          ...
          <div :class="`${!isOpenToogleNav ? 'invisible opacity-0 lg:visible lg:opacity-100' : 'block'} p-3 absolute py-5 bg-white shadow-lg rounded-lg max-w-[150px] w-full right-4 top-full lg:block lg:static lg:bg-transparent lg:max-w-full lg:shadow-none lg:rounded-none transition-all`">
            <ul class="flex flex-col gap-y-2 lg:flex-row lg:items-center md:gap-x-10">
              ...
              <li class="hover:text-slate-700 relative pb-1 lg:pb-0 border-b lg:border-0" >
                ...
                <div v-if="isOpenImageNav"
                  :class="`border rounded bg-white w-36 max-w-md absolute top-full  lg:mt-2 -ms-8 -left-full lg:ms-3 ${isOpenImageNav ? 'block' : 'hidden'}`" ref="imageRef">
                  <div v-for="(imageNav, i) in navigations.images" :key="i">
                    <NuxtLink :to="`/image/${imageNav.link}`"
              ...
              <li class="hover:text-slate-700 relative pb-1 lg:pb-0 border-b lg:border-0" >
                ...
                <div v-if="isOpenVideoNav"
                  :class="`border rounded bg-white w-36 max-w-md absolute top-full  lg:mt-2 -ms-8 -left-full lg:ms-3 ${isOpenVideoNav ? 'block' : 'hidden'}`" ref="videoRef">
  
                  <NuxtLink v-for="(videoNav, i) in navigations.videos" :to="`/video/${videoNav.link}`" :key="i"
                    :class="{ 'text-slate-600 bg-slate-50': i === hoveredIndex, 'p-1.5 block border-b': true }"
                    @mouseenter="handleMouseEnter(i)" @mouseleave="handleMouseLeave">
                    {{ videoNav.name }}
                  </NuxtLink>
